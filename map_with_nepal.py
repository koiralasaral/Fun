...
# For demonstration purposes, we will override the sample point with dummy local data.
# (In a complete run, one could use P_local = np.array([x_local[i_sample], y_local[i_sample]]) )
P_local = np.array([0.5, 0.3])   # Dummy local coordinates (x, y)
...
# Now compute ψ: angle between OP and T at P.
psi = np.arccos(np.dot(P_local, T_sample)/(r_sample+1e-9))
psi_deg = np.degrees(psi)

# Approximate dr/dp using central differences.
...
# Compute polar angle θ of OP.
theta_val = np.arctan2(P_local[1], P_local[0])
# For the osculating circle, compute curvature:
...
# The centre C of the osculating circle: C_local = P_local + R0 * N, where N is the unit normal.
...
# Compute vertical chord (PN) on the osculating circle.
...
# Compute horizontal chord (PM) on the osculating circle.
...
Q_global = local_to_global(np.dot(P_local, T_sample)*T_sample)
...
seg = 0.02  # in local coordinate units (degrees)
...
folium.PolyLine(locations=[[OP_coords[0][0], OP_coords[0][1]],
                           [OP_coords[1][0], OP_coords[1][1]]],
...
for theta_val_c in np.linspace(0, 2*np.pi, 100):
    ...
    # GeoJSON expects format [lat, lon]
...
# Add a marker at the sample point P with multiple popups showing computed values and formulas.
marker_popup = (
...
# Add a static marker showing all formulas near the map center.
formula_text = (
...
print("Folium map saved as 'map_with_full_geometry.html'. Open this file in your browser to view the complete annotated map.")
