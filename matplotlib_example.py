...
# ==============================================================================
# PART 1. Define Waypoints and Compute a Spline‐Smoothed Route (Global Coordinates)
# ==============================================================================
...
# Define a set of waypoints (global, as [lat, lon]) from Nepalgunj to Everest.
waypoints = np.array([
    nepalgunj,
    chitwan,
    kathmandu,
...
# Spline interpolation (cubic, with s=0 for exact interpolation).
tck, u = splprep([lons, lats], s=0, k=3)
u_new = np.linspace(0, 1, 200)
lon_smooth, lat_smooth = splev(u_new, tck)
# (Note: the order here is [lon, lat].)

# Also get first and second derivatives (used for tangent and curvature estimation)
...
# Using Nepalgunj as the origin O.
O_lat, O_lon = nepalgunj[0], nepalgunj[1]
# Define local coordinates for each point:
#   x_local = (global lon - O_lon) and  y_local = (global lat - O_lat)
x_local = lon_smooth - O_lon
y_local = lat_smooth - O_lat
r_arr = np.sqrt(x_local**2 + y_local**2)   # Radial distances (OP) for each sample.

# Compute unit tangent vectors from the first derivatives.
# (Since our route is in global coordinates, these derivatives are in terms of degrees.)
T_arr = np.vstack((dlon, dlat)).T
T_norm = np.linalg.norm(T_arr, axis=1)
T_unit = (T_arr.T / T_norm).T

# Compute pedal distances for each sample: p = |(P · T)|.
p_arr = []
for i in range(len(r_arr)):
    P_i = np.array([x_local[i], y_local[i]])
    T_i = T_unit[i]
    p_val = np.abs(np.dot(P_i, T_i))
    p_arr.append(p_val)
...
# Select the middle sample point automatically.
i_sample = len(u_new) // 2
P_local = np.array([x_local[i_sample], y_local[i_sample]])  # Local coordinates of sample point P.
r_sample = np.linalg.norm(P_local)                           # OP = r.
T_sample = T_unit[i_sample]                                   # Unit tangent at P.
p_sample = p_arr[i_sample]                                    # Pedal distance p.
