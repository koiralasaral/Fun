...
# Example data for the local geometry computation
# (Assume these were computed from your spline interpolation)
# For demonstration, we define simple dummy values

# Suppose we obtained local coordinates for P (sample point)
P_local = np.array([0.5, 0.3])  # local coordinate differences (x, y)
r_sample = np.linalg.norm(P_local)  # OP = r
# Assume unit tangent (T_sample) is computed as:
...
# Convert ψ from radians to degrees:
psi_deg = np.degrees(psi)

# For demonstration, create a marker popup using the angle values.
popup_text = "Tangent at P (ψ = {:.2f} rad, {:.1f}°)".format(psi, psi_deg)

# Create a simple Folium Map and add the marker.
m = folium.Map(location=[27.7, 85.3], zoom_start=8)
folium.Marker(location=[27.7, 85.3], popup=popup_text, 
